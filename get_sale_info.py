...
class get_sale_info(threading.Thread):
    ...
    def getSaleInfo(self):
       ...
       #查询所有房产信息
        query_house_license_sql = 'SELECT * FROM T_HOUSE_LICENSE'
        self.dbCursor.execute(query_house_license_sql)
        LicenseResultList =self.dbCursor.fetchall()
        for license in LicenseResultList:
            licenseId = license[0]
            detailUrl = license[11]
            try :
                # 获取预售基本信息
                self.getSaleBaseInfo(detailUrl, licenseId)
                # 获取每天房间销售明细信息
                self.getSaleDetailInfo(detailUrl, licenseId)
            except Exception as error:
                print("获取license,",licenseId,'销售信息失败。',error)
